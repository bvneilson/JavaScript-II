...
// Problem 1
// Every runner was feeling doubly generous upon arrival at the event. So we need to double each runner's donation.
let doubleDonos = runners.map((runner) => {
  runner.donation *= 2;
  return runner;
});
console.log(doubleDonos);

// Problem 2
// The event director wants info on any attendees whose email does not end in '.com'.
let weirdEmails = runners.filter((runner) => runner.email.includes(".com") === false);
console.log(weirdEmails);

// Problem 3
//The event director would like to know the average donation amount.
let totalDonations = runners.reduce((total, runner) => {
  return total += runner.donation;
}, 0);
let avgDonation = totalDonations / runners.length;
